...
class TD_MCTS:
    def __init__(self, env, approximator, iterations=500, exploration_constant=1.41, rollout_depth=10, gamma=0.99, normalization_factor=50000):
        self.env = env
    ...
    def select_child(self, node):
        ...
        if isinstance(node, TD_MCTS_After_Node):
            print("Node is TD_MCTS_After_Node")
            return None, None
        best_action = None
        best_child = None
    ...
    def rollout(self, sim_env, depth):
        ...
        if sim_env.is_game_over():
            print
            return 0
        original_score = sim_env.score
        final_state = sim_env.board.copy()
        ...
        legal_moves = [a for a in range(4) if sim_env.is_move_legal(a)]
        if not legal_moves:
            return ( final_score - original_score ) / self.normalization_factor
        else:
            best_value = -np.inf
            for action in legal_moves:
                after_state, reward = deterministic_step(final_state, action)
                value_estimate = reward + self.approximator.value(after_state)
                if value_estimate > best_value:
                    best_value = value_estimate
            return ( best_value + ( final_score - original_score )) / self.normalization_factor
    ...
    def expand(self, node):
        for action in node.untried_actions:
            after_state, reward = deterministic_step(node.state, action)
            node.children[action] = TD_MCTS_After_Node(after_state, (node.score + reward) / self.normalization_factor, parent=node, action=action)
            for _ in range(1):
                next_state = board_add_random_tile(after_state.copy())
                next_node = TD_MCTS_Node(next_state, (node.score + reward) / self.normalization_factor, parent=node)
                node.children[action].children.add(next_node)
                temp_env = self.create_env_from_state(next_state, node.score + reward)
                rollout_reward = self.rollout(temp_env, self.rollout_depth)
    ...
    def run_simulation(self, root):
        ...
        while node.fully_expanded():
            ...
            next_node = TD_MCTS_Node(next_state, score, parent=after_node)
...
def get_action(state, score):
    with open("ntuple_weights20000.pkl", "rb") as f:
    ...
    env = Game2048Env()
    td_mcts = TD_MCTS(env, approximator, iterations=50, exploration_constant=0.1, rollout_depth=0, gamma=1, normalization_factor=50000)
